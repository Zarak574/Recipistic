...
$searchBtn.addEventListener("click", function (){
    if($searchField.value) window.location =`/cook/recipes.php?q=${$searchField.value}`;
});
...
/** API - FETCH DATA FOR TAB CONTENT */
...
for(let i=0; i<12; i++){
 ...
 <h3 class="title-small">
     <a href="./detail.php?recipe=${recipeId}" class="card-link">${title ?? "Untitled"}</a>
 </h3>
...
$currentTabPanel.innerHTML += `
<a href="./recipes.php?mealType=${$currentTabBtn.textContent.trim().toLowerCase()}" class="btn btn-secondary label-large has-state">Show more</a>
`; 
...
for(const [index, $sliderSection] of $sliderSections.entries()){
    ...
    fetchData([ ...cardQueries, ["cuisineType", cuisineType[index]]], function(data) {
        ...
        <li class="slider-item" data-slider-item>
            <a href="./recipes.php?cuisineType=${cuisineType[index].toLowerCase()}" class="load-more-card has-state">
